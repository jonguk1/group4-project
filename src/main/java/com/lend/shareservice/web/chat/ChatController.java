...
import com.lend.shareservice.domain.chat.ChatDTO;
...
public class ChatController {
    ...
    // MessageMapping을 통해 webSocket로 돌아오는 메시지를 발신 처리
    // 이때 클라이언트에서는 /pub/chat/message로 요청하게 되고, 이것을 controller가 받아서 처리
    // 처리가 완료되면 /sub/chat/room/roomId로 메시지 전송
    ...
    public void sendMessage(@Payload ChatDTO chat)
    {
        ...
        template.convertAndSend("/send/chat/room/" + chat.getRoomId(), chat);
    }

    // 유저 퇴장 시에는 EventListener를 통해 유저 퇴장 확인
    @EventListener
    public void webSocketDisconnectListener(SessionDisconnectEvent event)
    {
        ...
        // stomp 세션에 있던 uuid와 roomId를 확인 후 채팅방 유저 리스트와 room에서 해당 유저 삭제
